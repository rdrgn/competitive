...
struct Bot {
  ...
  const Task& choice_task(int index) {
    vector<pair<double, int>> estimated_scores(game_info.task.size());
    for (int i = 0; i < game_info.task.size(); i++) {
      const auto& e = game_info.task[i];
      if (e.s.size() == 1) continue;

      auto p = agent_last_point[index];
      int t = 0;
      for (auto c : e.s) {
        auto cp = get_checkpoint(c);
        t += max(100, int(100 * sqrt(pow(p.first - cp.first, 2) + pow(p.second - cp.second, 2))));
        if (p == cp) t += 100;
        p = cp;
      }
      estimated_scores[i].first =
          ((double)(e.count + 1) / (e.total + 1) - (e.count == 0 ? 0 : (double)e.count / e.total)) * e.weight / t;
      estimated_scores[i].second = i;
    }
    std::sort(estimated_scores.rbegin(), estimated_scores.rend());

    int r = estimated_scores[index].second;
