...
struct Bot {
  ...
  const string choice_task_s(int index) {
    const auto& history = game_info.agent[index].history;

    vector<pair<double, string>> merged_task_ss;
    merged_task_ss.reserve(game_info.task.size() * game_info.task.size());
    for (const auto& t : game_info.task) {
      if (t.s.size() == 1) continue;
      merged_task_ss.push_back({(double)t.weight / (t.total + 1), t.s});
    }
    for (int i = 0; i < game_info.task.size(); i++) {
      if (game_info.task[i].s.size() == 1) continue;
      for (int j = 0; j < game_info.task.size(); j++) {
        if (i == j) continue;
        if (game_info.task[j].s.size() == 1) continue;
        const auto& ti = game_info.task[i];
        const auto& tj = game_info.task[j];

        for (int k = min(ti.s.size() - 1, tj.s.size() - 1); k >= 1; k--) {
          if (ti.s.substr(ti.s.size() - k) == tj.s.substr(0, k)) {
            double v = (double)ti.weight / (ti.total + 1) + (double)tj.weight / (tj.total + 1);
            auto s = ti.s + tj.s.substr(k);
            merged_task_ss.push_back({v, s});
            break;
          }
        }
      }
    ...
    for (auto& e : merged_task_ss) {
      for (int k = min(e.second.size() - 1, history.size()); k >= 1; k--) {
        if (history.substr(history.size() - k) == e.second.substr(0, k)) {
          e.second = e.second.substr(k);
        }
      }

      auto p = agent_last_point[index];
      int t = 0;
      for (int j = 0; j < e.second.size(); j++) {
        auto cp = get_checkpoint(e.second[j]);
        t += max(100, int(100 * sqrt(pow(p.first - cp.first, 2) + pow(p.second - cp.second, 2))));
        if (p == cp) t += 100;
        p = cp;
      }
      e.first /= t;
    }
    std::sort(merged_task_ss.rbegin(), merged_task_ss.rend());

    return merged_task_ss[index].second;
  }
  pair<int, int> get_checkpoint(char c) { return master_data.checkpoints[c - 'A']; }
  // 移動予定を設定
  void set_move_point(int index) {
    const auto next_task_s = choice_task_s(index);
    cerr << "Agent#" << index + 1 << " next task:" << next_task_s << endl;

    for (char c : next_task_s) {
      ...
      if (before_point == move_point) {
        int tmp_x = before_point.first < master_data.area_size / 2 ? before_point.first + 1 : before_point.first - 1;
        int tmp_y = before_point.second;
